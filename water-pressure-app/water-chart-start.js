...
WaterPressureApp.init('#water-pressure-app', {
    ...
    //获取水压数据
    loadData: function loadData(ryc, hrz, time) {
    ...
    //点标签提示
    pointLabel: function pointLabel(data, type) {
        return data.name;
    },

    //点标签样式
    pointLabelStyle: function pointLabelStyle(data, type) {
        return {
            color: '#1488C4',
            fontSize: 20,
            fontStyle: 'normal',
    ...
    //点漂浮提示
    pointTip: function pointTip(data, type) {
        return labels[type] + '：' + data[type];
    },

    //点样式
    pointStyle: function pointStyle(data, type) {
        return {
            symbolSize: 15,
            normal: {
    ...
    //线漂浮提示
    lineTip: function lineTip(current, next, type) {
        if (next) {
    ...
    //线样式
    lineStyle: function lineStyle(current, next, type) {
        return {
            normal: {
                width: 6,
                opacity: type == 'standardHigh' || type == 'standardLow' ? 0.7 : 1
