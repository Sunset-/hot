    ...
    function saveAll() {
        Object.keys(DATA_MAP_REL).forEach(key => {
            var list = DATA[DATA_MAP_REL[key]];
            if (list) {
                if (!~key.indexOf('PIPE')) {
                    localStorage.setItem(`${key}_DATA`, JSON.stringify(list.map(item => {
                        return {
                            type: item.type,
                            coord: [item._marker.point.lng, item._marker.point.lat],
                            region: item._region && item._region.getPath().map(p => `${p.lng},${p.lat}`).join(';'),
                            data: item.data
                        };
                    })));
                } else {
                    localStorage.setItem(`${key}_DATA`, JSON.stringify(list.map(item => {
                        return {
                            type: item.type,
                            path: item._line.getPath().map(point => `${point.lng},${point.lat}`).join(';'),
                            data: item.data
                        };
                    })));
                }
